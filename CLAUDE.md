...
| 子任务 | 状态 | 说明 |
|-------|------|------|
| 1.1 工程初始化 | ✓ | 依赖安装，后端/前端均能正常编译运行 |
| 1.2 TypeORM + MySQL | ✓ | 7 张实体表，DatabaseModule，synchronize: true |
| 1.3 Auth 适配 | ✓ | JWT + Passport → MySQL，种子脚本创建默认管理员 |
| 1.4 Customers 迁移 | ✓ | 自动编号、防重、删除校验、订单统计，API 测试通过 |
| 1.5 前端路由调整 | ✓ | 移除 Leads/Activities/Tasks，新增 Products/Orders |
| 1.6 数据迁移脚本 | ✓ | `npm run migrate`，JSON→MySQL，含重复名处理 |
| 1.7 集成测试 | ✓ | 21/21 测试通过（Auth + Customers CRUD 全流程） |

### Phase 2 完成总结

| 子任务 | 状态 | 说明 |
|-------|------|------|
| 2.1 产品管理模块 | ✓ | 后端 CRUD API + 启用/停用 + 名称搜索 + 删除引用校验；前端列表页（搜索/分页/状态切换）|
